...
function checkEntityCollision() { // where ent is, if snake head there hit ent, upd n score points in arr,
    ...
    if (hit.type === 'hole') {
        if (!hit.blocked && !hit.inUse) teleportSnake(hit); // only entrance leaves
    }
    if (hit.respawn){
...
function teleportSnake(entrance) {
    ...
    if (holeCD > 0) return;
    if (entrance.inUse || entrance.blocked) return;
    ...
    entrance.inUse = true;
    entrance.blocked = true; // only take out the entrance
...
function tickHoleTimers() {
    entities.forEach(ent => {
        ...
        return true; // keep the other one
    });
}
